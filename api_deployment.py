...
class ImageClassifier(nn.Module):
    ...
    def predict(self, out_features):
        with torch.no_grad():
            x = self.forward(out_features)
            return x
    
    def predict_proba(self, out_features):
        with torch.no_grad():
            x = self.forward(out_features)
            return torch.softmax(x, dim=1)

    def predict_classes(self, out_features):
        with torch.no_grad():
            x = self.forward(out_features)
...
img_classifer = ImageClassifier(num_classes=len(image_decoder), decoder = image_decoder, device='cpu')
